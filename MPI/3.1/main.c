...
int main(int argc, char *argv[])
{
    ...
    // Configuración de los contenedores y generación de datos
...
// Verificación de errores en las llamadas a MPI
void e(int error)
{
...
void Print_histo(float bin_maxes[], int bin_counts[], int bin_count, float min_meas)
{
    ...
    // Imprimir cada contenedor con su representación gráfica
    for (i = 0; i < bin_count; i++)
    {
...
// Encontrar el contenedor adecuado para cada dato
void Find_bins(int bin_counts[], float local_data[], int loc_bin_cts[],
               int local_data_count, float bin_maxes[], int bin_count,
               float min_meas, MPI_Comm comm)
{
...
// Determinar el contenedor correspondiente a un dato
int Which_bin(float data, float bin_maxes[], int bin_count, float min_meas)
{
    int i;
               ...
               int comm_sz, MPI_Comm comm)
{
    if (my_rank == 0)
    {
        ...
        // Asegurar que data_count sea múltiplo del tamaño del comunicador
